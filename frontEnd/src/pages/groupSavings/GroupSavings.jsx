...
function GroupSavings() {
  ...
  if (event == undefined)
    return (
      <div className="groupchat-centered full-height">
    ...
    <>
      ...
      <div className="savings-card">
        ...
        <p className="savings-amount">
          $ {compoundInterest(event?.savings?.months)}.00
        </p>
        <div className="chart">
          {graphData.map((data, i) => (
            <div key={i} className="bar" style={{ height: data + "px" }}></div>
          ))}
        </div>
        <p className="savings-detail">
          Over the next {event?.savings?.months} months
        </p>
      </div>
    </>
  );
